<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            USIM 차단 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            USIM 차단
        </v-card-title >        
...
<script>
    ...
    export default {
        name: 'ServicemanagementUsimBlock',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/usimBlocks'), this.value)
                        } else {
