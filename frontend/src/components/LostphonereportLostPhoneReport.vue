<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            분실신고 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            분실신고
        </v-card-title >        
...
<script>
    ...
    export default {
        name: 'LostphonereportLostPhoneReport',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/lostPhoneReports'), this.value)
                        } else {
