<template>
    <v-card outlined>
        ...
        <v-card-title v-if="value._links">
            IMEI 차단 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            IMEI 차단
        </v-card-title >        
...
<script>
    ...
    export default {
        name: 'ServicemanagementImeiBlock',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/imeiBlocks'), this.value)
                        } else {
